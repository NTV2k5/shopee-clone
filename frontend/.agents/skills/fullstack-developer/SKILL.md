...
### Tailwind CSS v4 + Next.js 15/16
If you encounter `Error evaluating Node.js code` or `Missing field negated on ScannerOptions.sources`:
1. **Update to 4.1.0+**: This error is caused by a version mismatch in the 4.0.x series. Ensure all tailwind packages are at least `4.1.0`.
   ```bash
   npm install tailwindcss@latest @tailwindcss/postcss@latest
   ```
2. **Clean Install**: If the error persists, delete `node_modules` and `package-lock.json` before reinstalling.
3. **PostCSS Configuration**: Ensure `postcss.config.mjs` uses `@tailwindcss/postcss`.
   ```javascript
   export default {
     plugins: {
       '@tailwindcss/postcss': {},
